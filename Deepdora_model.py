...
pept_seqs = data_prep.get_peptides_from_csv('data/netMHCpan_set/NetMHCpan_train/ba_complete.csv', 0, 2, ' ', skip_first_line = False)

accepted_MHCs = ['HLA', 'H2', 'RT1', 'SLA', 'BF2', 'Eqca']

nocell_pept_seqs = []
for pept in pept_seqs:
    for MHC in accepted_MHCs:
        if pept[1].startswith(MHC):
            nocell_pept_seqs.append(pept)
            break
        elif pept[1].startswith('H-2'):
            nocell_pept_seqs.append((pept[0], 'H2' + pept[1][3:]))
        elif pept[1].startswith('Mamu'):
            nocell_pept_seqs.append((pept[0], pept[1][:-2] +':'+ pept[1][-2:]))
        elif pept[1].startswith('BoLA'):
            nocell_pept_seqs.append((pept[0], pept[1].replace(':', '*')))


pept_seqs = nocell_pept_seqs
###
...
def na_model(k, pept_seq):
    ...
    if allele.startswith('HLA'):      # Human
        ...
        if allele in allele_ID.keys():
            pass
        elif allele.split(':')[0] in allele_ID.keys():
...
non_modelled = Parallel(n_jobs = n_cores)(delayed(na_model)(k+pepts_start, pept_seq) for k, pept_seq in enumerate(pept_seqs[pepts_start:pepts_end]))
